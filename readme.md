# Tezos Synapse Docker

This Docker image will run Synapse as a single process and is a fork of the matrixdotorg/synapse docker image. Also large portions of this readme have been forked/copied from https://github.com/matrix-org/synapse/blob/master/docker/README.md since it mostly shares the exact same features.
...
By default it uses a sqlite database; for production use you should connect it to a separate postgres database.
...
docker run -it --rm \
    ...
    airgapdocker/tezos-synapse:v1.5.0-py3 generate
```
...
docker run -it --rm \
    ...
    -e SYNAPSE_SERVER_NAME=my.matrix.host \
    -e SYNAPSE_REPORT_STATS=yes \
    airgapdocker/tezos-synapse:v1.5.0-py3 migrate_config
```
